...
// ── Legacy backfill ──────────────────────────────────────────────────────────
// `checkout_leads` was the original (client-SDK) capture: phone + created_at only,
// no session, no name/address/cart. Nothing ever read it back, so those months of
// phone numbers are invisible. Copy them into `people` so they show up in /admin.
// Never overwrites richer data from the live flow — `people` fields win on merge.
export async function backfillLegacyLeads(): Promise<{ scanned: number; written: number } | null> {
  const db = adminDb();
  if (!db) return null;
  const snap = await db.collection('checkout_leads').get();
  let written = 0;
  // Oldest first, so the earliest created_at ends up as firstSeen.
  ...
  for (const v of docs) {
    const key = personKey(v.phone as string);
    if (!key) continue;
    const at = v.created_at instanceof Timestamp ? v.created_at : null;
    const ref = db.collection('people').doc(key);
    const existing = await ref.get();
    const patch: Record<string, unknown> = { phone: v.phone, phoneAt: at, lastSeen: at };
    if (v.email) patch.email = v.email;
    // Only claim firstSeen/source for people we've never seen in the live flow.
    if (!existing.exists) { patch.firstSeen = at; patch.source = 'legacy-checkout'; }
    else {
      // Keep the earliest firstSeen and the latest lastSeen across both sources.
      const e = existing.data() as Record<string, unknown>;
      const ef = e.firstSeen instanceof Timestamp ? e.firstSeen.toMillis() : Infinity;
      const el = e.lastSeen instanceof Timestamp ? e.lastSeen.toMillis() : 0;
      if (at && at.toMillis() >= ef) delete patch.firstSeen;
      if (at && at.toMillis() <= el) delete patch.lastSeen;
    }
    await ref.set(patch, { merge: true });
    written++;
  }
